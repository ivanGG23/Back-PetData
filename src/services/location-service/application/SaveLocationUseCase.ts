...
export class SaveLocationUseCase {
  ...
  async execute(data: CreateLocationRequest): Promise<Location> {
    if (!data.reporte_id || !data.latitud || !data.longitud || !data.precision_metros) {
      throw new Error("Todos los campos son requeridos: reporte_id, latitud, longitud, precision_metros");
    }
